...
%for experiment 9, in which the fly gets short stripe fixation trials
%alternated with optomotor response trials
close all; clear all;

% prompt the user to select the file to open and load it.
cd 'Z:\Wilson Lab\Mel\Experiments\Exp9\data'
[file,path] = uigetfile();
load([path,file]);
...
for i = 1:length(stripeFixationTrials)
    [smoothedSF{i}] = singleTrialVelocityAnalysis9mm(dataSF{1,i},sampleRate);
end

for i = 1:length(clockwiseTrials)
    [smoothedClockwise{i}] = singleTrialVelocityAnalysis9mm(dataClockwise{1,i},sampleRate);
end

for i = 1:length(counterclockwiseTrials)
    [smoothedCounterclockwise{i}] = singleTrialVelocityAnalysis9mm(dataCounterclockwise{1,i},sampleRate);
end
...
for i = 1:length(smoothedClockwise)
    forwardVelocityClockwise{i} = smoothedClockwise{i}.xVel(1:74);
end
...
for i = 1:length(smoothedCounterclockwise)
    forwardVelocityCounterclockwise{i} = smoothedCounterclockwise{i}.xVel(1:74);
end
...
%(2) Clockwise optomotor trials
for i = 1:length(smoothedClockwise)
    angularVelocityClockwise{i} = smoothedClockwise{i}.angularVel(1:74);
end

angularVelocityClockwise = cell2mat(angularVelocityClockwise);
angularVelocityClockwise = reshape(angularVelocityClockwise,[length(smoothedClockwise{1}.angularVel(1:74)),length(angularVelocityClockwise)/length(smoothedClockwise{1}.angularVel(1:74))]);

meanVelocityClockwise = mean(angularVelocityClockwise,2);

%(3) Counterclockwise optomotor trials
for i = 1:length(smoothedCounterclockwise)
    angularVelocityCounterclockwise{i} = smoothedCounterclockwise{i}.angularVel(1:74);
end

angularVelocityCounterclockwise = cell2mat(angularVelocityCounterclockwise);
angularVelocityCounterclockwise = reshape(angularVelocityCounterclockwise,[length(smoothedCounterclockwise{1}.angularVel(1:74)),length(angularVelocityCounterclockwise)/length(smoothedCounterclockwise{1}.angularVel(1:74))]);
