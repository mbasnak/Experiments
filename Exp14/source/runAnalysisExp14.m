...
cd 'Z:\Wilson Lab\Mel\Experiments\Exp14\data'
path = 'Z:\Wilson Lab\Mel\Experiments\Exp14\data\';
        ...
        for j = 1:length(flies)
            if strfind(flies(j).name,'flyNum') ~=0
                ...
                for t = 1:length(trials)
                    if (strfind(trials(t).name,'Block') ~=0) 
                                  
                            singleTrialAnalysisExp14(NewPath,trials(t).name);
