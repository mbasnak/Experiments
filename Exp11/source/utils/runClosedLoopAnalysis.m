...
%This function analyses single trials of closed-loop data

%OUTPUT
%smoothed = has the smoothed position velocity information from FicTrac's
%data
%posToRadFly = fly's heading in radians
%degsFlyDistMoving = this is basically just a vector from -180 to 180 with
%the potential headings a fly might adopt
%probabilitiesDistMoving = Distance to the goal of the fly throughtout the
%experiment, quantified as a probability that in the frames in which the
%animal is moving it is headed in each angle with respect to its mean
%heading


%INPUT
%rawData = the data to be analyzed
%trialName = the name and number of the trial.
...
sampleRate = 1000; %sampling rate of the NiDaq

%Next are two different ways of getting the smoothed position and velocity
%information
...
% forward velocity. If it's above 0.5 mm/s we will consider it is moving
...
%Here, I'm changing the output of the panels from pixels to degrees, while
%also accounting for the fact that xpos=70 is the one right in front of my
%fly.
...
%I'm definining the 'goal' as the circular mean of the fly's heading
