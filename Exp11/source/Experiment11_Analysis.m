...
cd 'Z:\Wilson Lab\Mel\Experiments\Exp11\data'
[file,path] = uigetfile();
load([path,file]);
...
suptitle('Distribution of heading for light stripes');
...
suptitle('Distribution of heading for dark stripes');
...
% Get the angular velocity data during clockwise and counterclockwise
% presentations
for i = 1:length(FastClockOpenLoopIndex)
...
% Plot the angular velocity as a function of the bar position for both turn
% types.
figure('Position',[200 200 1000 600]),
subplot(1,2,1)
p1 = boundedline(remapPosToDegFC{1,1}(3:end),meanAngVelFC(3:end),stdFC(3:end)/sqrt(i),'k','alpha')
hold on
