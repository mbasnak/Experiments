...
for x = 8:49 % for each frame in the x axis
...
for x = 56:96 % for each frame in the x axis
    Pats(:,:,x,:) = circshift(Pats(:,:,x-1,:),[0,1]); % make the value of the frame equal to moving the 3rd dimension by one pixel
end
...
writerObj = VideoWriter('Z:\Wilson Lab\Mel\FlyOnTheBall\data\Experiment15\stimVideos\rotationalFlowGrating.avi');
...
for y = 2:pattern.y_num
        ...
        %first window
        Pats(:,22:69,[1:7,50:55],y) = circshift(Pats(:,22:69,[1:7,50:55],y-1),[0,-1]);
        Pats(:,70:93,[1:7,50:55],y) = circshift(Pats(:,70:93,[1:7,50:55],y-1),[0,1]);
        % The next 3 lines prevent the formation of a smaller or larger bar
        % apearing when the stimulus moves.
        Pats(:,94,[1:7,50:55],y) = Pats(:,93,[1:7,50:55],y-1);
        Pats(:,95,[1:7,50:55],y) = Pats(:,94,[1:7,50:55],y-1);
        Pats(:,96,[1:7,50:55],y) = Pats(:,95,[1:7,50:55],y-1);
        
        Pats(:,1:18,[1:7,50:55],y) = circshift(Pats(:,1:18,[1:7,50:55],y-1),[0,1]);
        Pats(:,19,[1:7,50:55],y) = Pats(:,18,[1:7,50:55],y-1);
        Pats(:,20,[1:7,50:55],y) = Pats(:,19,[1:7,50:55],y-1);
        Pats(:,21,[1:7,50:55],y) = Pats(:,20,[1:7,50:55],y-1); 
                       
        
        %Leave the grating static for the other x dimensions when moving in
        %y
        Pats(:,:,8:49,y) = Pats(:,:,8:49,1);  
        Pats(:,:,56:96,y) = Pats(:,:,56:96,1);
...
% create the video writer with 1 fps
writerObj = VideoWriter('Z:\Wilson Lab\Mel\FlyOnTheBall\data\Experiment15\stimVideos\FtoBgrating.avi');
writerObj.FrameRate = 1; % set the seconds per image
% open the video writer
open(writerObj);
% write the frames to the video
...
directory_name = 'C:\Users\Wilson\Desktop\panels-matlab_071618\Patterns\mel360_36panels';
 str = [directory_name '\Pattern029_grating_360_XY_opticflow180deg_30degwindow']; 	% name must begin with ‘Pattern_’
 save(str, 'pattern');