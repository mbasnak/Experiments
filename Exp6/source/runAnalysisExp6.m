...
cd 'Z:\Wilson Lab\Mel\Experiments\Exp6\data'
path = 'Z:\Wilson Lab\Mel\Experiments\Exp6\data\';
        ...
        for j = 1:length(flies)
            if strfind(flies(j).name,'flyNum') ~=0
                ...
                for t = 1:length(trials)
                    if strfind(trials(t).name,'Yoked') ~=0
                
                    % Run the analysis for the bar jump data                 
                        yokedTrialAnalysisExp6(NewPath,trials(t).name); 
                    
                    else
                       if strfind(trials(t).name,'Error') ~=0 
                    
                            singleTrialAnalysisExp6(NewPath,trials(t).name);
                       end
