...
folderNames = dir('Z:\Wilson Lab\Mel\Experiments\Uncertainty\Exp30\data\high_contrast');
...
for folder = 1:length(folderNames)
   if contains(folderNames(folder).name,'60D05') == 1
       ...
       change_offset = [change_offset;change_in_offset_AJ];
       offset = [offset;offset_AJ];
       longer_offset = [longer_offset;longer_offset_AJ];
       first_frame = [first_frame;zscore(first_return_frame')];
       partial_frame = [partial_frame;zscore(partial_return_frame')];
       half_frame = [half_frame;zscore(half_return_frame')];
       zscored_BM = [zscored_BM,zscore(mean_bm_pre_jump)];
       zscored_BW = [zscored_BW,zscore(mean_bw_pre_jump)];
   end
end

path = 'Z:\Wilson Lab\Mel\Experiments\Uncertainty\Exp30\data\high_contrast\group_plots';
%% Plot

figure('Position',[100 100 1000 800]),
imagesc(change_offset)
colorbar
...
saveas(gcf,[path,'\change_in_offset_AJ.png'])
...
% xtlbl = linspace(min(time_around_jump), max(time_around_jump), numel(xt));                    
% set(gca,'XTick',xt-4.5, 'XTickLabel',round(xtlbl,2))  
title('Change in offset around the jumps, sorted by BM');
xlabel('Time around the jumps (frames)');
ylabel('Sorted jump #');

%get indexes for sorted BW
[sorted_Bw,Iw] = sort(zscored_BW);

subplot(1,2,2),
imagesc(change_offset(Iw,:))
colorbar
colormap(flipud(gray))
hold on
xline(45.5,'linewidth',2,'color','r');
...
% xtlbl = linspace(min(time_around_jump), max(time_around_jump), numel(xt));                    
% set(gca,'XTick',xt-4.5, 'XTickLabel',round(xtlbl,2))  
title('Change in offset around the jumps, sorted by BW');
xlabel('Time around the jumps (frames)');
ylabel('Jump #');

saveas(gcf,[path,'\change_in_offset_AJ_sorted.png'])
...
figure('Position',[100 100 1000 800]),
subplot(1,2,1)
imagesc(change_offset(I,36:56))
colorbar
colormap(flipud(gray)) 
hold on
xline(10.5,'linewidth',2,'color','r');
title('Change in offset around the jumps, sorted by BM, close-up');
xlabel('Time around the jumps (frames)');
ylabel('Sorted jump #');

subplot(1,2,2)
imagesc(change_offset(Iw,36:56))
colorbar
colormap(flipud(gray))
hold on
xline(10.5,'linewidth',2,'color','r');
title('Change in offset around the jumps, sorted by BW, close-up');
xlabel('Time around the jumps (frames)');
ylabel('Sorted jump by BW');

saveas(gcf,[path,'\change_in_offset_AJ_sorted_closeup.png'])


%% Plot relationship between bump parameters and offset back to stable

summed_change_in_offset = change_offset(:,47) + change_offset(:,48) + change_offset(:,49) + change_offset(:,50);

figure,
subplot(1,2,1)
plot(zscored_BM,summed_change_in_offset,'ro')
bm_corr = corrcoef(zscored_BM,summed_change_in_offset);
xlabel('Zscored bump magnitude');
ylabel('Summed change in offset after cue jump (deg)');
text(0,180,['corr = ',num2str(round(bm_corr(2,1),2))])

subplot(1,2,2)
plot(zscored_BW,summed_change_in_offset,'ko')
bw_corr = corrcoef(zscored_BW,summed_change_in_offset);
xlabel('Zscored bump width');
text(0,180,['corr = ',num2str(round(bw_corr(2,1),2))])

saveas(gcf,[path,'\summed_changed_in_offset.png'])
...
shifted_offset_short = wrapTo180(offset-offset(:,26));
%short_time_AJ = time_around_jump(376:426);
