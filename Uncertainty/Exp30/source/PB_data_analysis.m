...
if (contains(parentDir,'changing_contrast')==1)
    [smoothed, bdata_time_out, stim_pos, flyPosRad, jump_frames, contrast] = get_data_360_new_settings(bdata_time, bdata_raw);    
else
...
if ~isempty(contrast)
    contrast_ds = contrast(round(linspace(1, length(contrast), volumes)));
    jump_frames_ds = jump_frames.*length(total_mvt_ds)/length(bdata_time);
else
    contrast_ds = [];
    jump_frames_ds = [];
end
...
data.jump_frames_ds = jump_frames_ds;
data.contrast_ds = contrast_ds;
