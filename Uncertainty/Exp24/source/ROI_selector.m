...
%glomerulus_numbering = [1:8;9:16];

%roi_i.num = glomerulus_numbering(~leftPB+1, glomerulus);
roi_i.name = name;
roi_i.z = zplane;
...
%roi_i.leftPB = leftPB;
%roi_i.rightPB = ~leftPB;
%roi_i.glomerulus = glomerulus;
...
% --- Executes during object creation, after setting all properties.
% function glomerulus_number_CreateFcn(hObject, eventdata, handles)
% % hObject    handle to glomerulus_number (see GCBO)
% % eventdata  reserved - to be defined in a future version of MATLAB
% % handles    empty - handles not created until after all CreateFcns called
% 
...
function addROI(hObject, ghandles, handles, roi)
    ...
    %ROI_list_current{end+1} = ['ID = ' num2str(roi.id,'%02d') ' | ' leftright{roi.rightPB+1} ' glom ' num2str(roi.glomerulus) ' | z ' num2str(roi.z)];
...
% % hObject    handle to leftPB (see GCBO)
% % eventdata  reserved - to be defined in a future version of MATLAB
% % handles    structure with handles and user data (see GUIDATA)
% ghandles = guihandles(hObject);
% guidata(hObject, handles);
% % Hint: get(hObject,'Value') returns toggle state of leftPB
% 
% 
% % --- Executes on button press in rightPB.
% function rightPB_Callback(hObject, eventdata, handles)
% % hObject    handle to rightPB (see GCBO)
% % eventdata  reserved - to be defined in a future version of MATLAB
% % handles    structure with handles and user data (see GUIDATA)
% ghandles = guihandles(hObject);
% guidata(hObject, handles);

% Hint: get(hObject,'Value') returns toggle state of rightPB
