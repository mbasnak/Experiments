...
for folder = 1:length(folderNames)
    if (contains(folderNames(folder).name,'60D05')==1 & contains(folderNames(folder).name,'20210127')==0 & contains(folderNames(folder).name,'20210218_60D05_7f_fly2')==0) %I'm excluding the first fly because I didn't run a darkness trial for it
        ...
        if type_of_fly == 1
            ...
            [var gain_change_trial_offset_var] = circ_std(gain_change_heading_offset(blockLimits{2}(1):blockLimits{2}(2)));
            gain_change_offset_var = [gain_change_offset_var,gain_change_trial_offset_var];
        else
            empty_trial_bar_offset = circ_dist(deg2rad(empty_trial.continuous_data.visual_stim_pos),empty_trial.continuous_data.bump_pos');            
            [var empty_trial_offset_var] = circ_std(empty_trial_bar_offset);
            empty_offset_var = [empty_offset_var,empty_trial_offset_var];
            gain_change_bar_offset = circ_dist(deg2rad(gain_change.continuous_data.visual_stim_pos),gain_change.continuous_data.bump_pos');                        
            [var gain_change_trial_offset_var] = circ_std(gain_change_bar_offset(blockLimits{2}(1):blockLimits{2}(2))); 
            gain_change_offset_var = [gain_change_offset_var,gain_change_trial_offset_var];
        end
...
for folder = 1:length(folderNames)
    if (contains(folderNames(folder).name,'60D05')==1 & contains(folderNames(folder).name,'20210127')==0 & contains(folderNames(folder).name,'20210218_60D05_7f_fly2')==0) %I'm excluding the first fly because I didn't run a darkness trial for it
        ...
        if type_of_fly == 1
            ...
            empty_offset_var = [empty_offset_var,empty_trial_offset_var];
            gain_change_heading_offset = circ_dist(-gain_change.continuous_data.heading,gain_change.continuous_data.bump_pos');
            [var gain_change_trial_offset_var] = circ_std(gain_change_heading_offset(blockLimits{2}(2)-2500:blockLimits{2}(2)));
            gain_change_offset_var = [gain_change_offset_var,gain_change_trial_offset_var];
        else
...
saveas(gcf,'C:\Users\Melanie\Dropbox (HMS)\Manuscript-Basnak\InvertedGain-Experiment\empty_trial_offset_comparison_typeI_lastthird.svg');
