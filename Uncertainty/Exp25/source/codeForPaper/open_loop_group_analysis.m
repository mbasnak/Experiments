...
mdl_offset = fitlme(allSummaryData,'stim_offset_var~contrast_level+(1|Fly)')

%% Get and plot mean offset variation per fly and in total

figure('Position',[200 200 1000 800]),
%Get mean offset var by contrast per fly
mean_stim_offset_data_per_fly = varfun(@mean,allSummaryData,'InputVariables','stim_offset_var',...
       'GroupingVariables',{'contrast_level','Fly'});
allFlies = [];
for fly = 1:length(data)
    fly_data{fly} = [mean_stim_offset_data_per_fly.mean_stim_offset_var(fly),mean_stim_offset_data_per_fly.mean_stim_offset_var(fly+length(data))];
...
ylim([min(mean_stim_offset_data_per_fly.mean_stim_offset_var)-0.3 max(mean_stim_offset_data_per_fly.mean_stim_offset_var)+0.3]);
...
mean_stim_offset_data_per_speed = varfun(@mean,allSummaryData,'InputVariables','stim_offset_var',...
       'GroupingVariables',{'contrast_level','stim_vel'});
plot(mean_stim_offset_data_per_speed.stim_vel(mean_stim_offset_data_per_speed.contrast_level==56),mean_stim_offset_data_per_speed.mean_stim_offset_var(mean_stim_offset_data_per_speed.contrast_level==56),'-o','lineWidth',2,'color',[0 0 0.6],'MarkerFaceColor',[0 0 0.6],'MarkerSize',8)
%add lines per fly
mean_stim_offset_data_per_speed_and_fly = varfun(@mean,allSummaryData,'InputVariables','stim_offset_var',...
       'GroupingVariables',{'contrast_level','stim_vel','Fly'});
allFlies_data{1} = [];
allFlies_data{2} = [];
for fly = 1:length(data)
    fly_offset_data{fly} = [mean_stim_offset_data_per_speed_and_fly.mean_stim_offset_var(mean_stim_offset_data_per_speed_and_fly.contrast_level==56 & mean_stim_offset_data_per_speed_and_fly.Fly==fly),mean_stim_offset_data_per_speed_and_fly.mean_stim_offset_var(mean_stim_offset_data_per_speed_and_fly.contrast_level==57 & mean_stim_offset_data_per_speed_and_fly.Fly==fly)];
    allFlies_data{1} = [allFlies_data{1},fly_offset_data{fly}(:,1)]; 
    allFlies_data{2} = [allFlies_data{1},fly_offset_data{fly}(:,2)]; 
end

hold on
plot(mean_stim_offset_data_per_speed.stim_vel(mean_stim_offset_data_per_speed.contrast_level==57),mean_stim_offset_data_per_speed.mean_stim_offset_var(mean_stim_offset_data_per_speed.contrast_level==57),'-o','lineWidth',2,'color',[ 0.5 0.8 0.9],'MarkerFaceColor',[ 0.5 0.8 0.9],'MarkerSize',8)
xlim([0 4]);
xticks(1:3);
...
set(gca,'XTickLabel',a,'fontsize',12,'FontWeight','bold')
ylabel('Offset variability (rad)','FontSize',12);
ylim([min(mean_stim_offset_data_per_speed.mean_stim_offset_var)-0.3 max(mean_stim_offset_data_per_speed.mean_stim_offset_var)+0.3]);

%save figure
saveas(gcf,[path,'\continuousGroupPlots\open_loop_offset_var_per_speed.png']);
