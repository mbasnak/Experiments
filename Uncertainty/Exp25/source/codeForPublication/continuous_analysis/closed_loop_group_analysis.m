...
allSummaryData = array2table(zeros(0,5),'VariableNames', {'contrast','offset_var','mean_new_bump_mag','mean_new_half_width','heading_var'});
flyNumber= [];
...
%Combine the tables into one
allModelData = array2table(zeros(0,15),'VariableNames', {'ContrastLevel','ForVelocity','ZscoredForVel','SideSpeed','ZscoredSideSpeed','YawSpeed','ZscoredYawSpeed','TotalMovement','ZscoredTotalMovement','Time','NewBumpMagnitude','AdjRSquare','NewBumpWidth','zscoredNewBM','zscoredNewBW'});
Fly = [];
for fly = 1:length(data)
...
%Save figure
saveas(gcf,[path,'\continuousGroupPlots\modelComparisonNewBM.png']);
...
%Save figure
saveas(gcf,[path,'\continuousGroupPlots\mean_closed_loop_new_bump_mag.png']);
...
%Save figure
saveas(gcf,[path,'\continuousGroupPlots\NewBumpMag_vs_mvt_and_contrast.png']);
...
%Save figure
saveas(gcf,[path,'\continuousGroupPlots\zNewBumpMag_vs_mvt_and_contrast.png']);
