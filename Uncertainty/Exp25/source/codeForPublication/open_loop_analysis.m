...
%     bar_position_to_plot(change_bar_position>40==1) = NaN;
    [x_out, bar_position_to_plot] = removeWrappedLines(data{1,session}.data.time, bar_position);
...
%     phase_to_plot(change_phase>40==1) = NaN;
    [x_out, phase_to_plot] = removeWrappedLines(data{1,session}.data.time, phase);
...
%     stimOffsetToPlot(changeStim_offset>100==1) = NaN;
    [x_out, stimOffsetToPlot] = removeWrappedLines(data{1,session}.data.time, stimoffset);
    subplot(3,5,[11 14])
    %Plot using different colors if the stimulus was low or high contrast
    if data{1,session}.data.run_obj.pattern_number == 57
        plot(x_out,stimOffsetToPlot,'color',[ 0.5 0.8 0.9],'LineWidth',1.5)
    else
        plot(x_out,stimOffsetToPlot,'color',[0 0 0.6],'LineWidth',1.5)
    end  
...
%% Fit model with two alphas - I'm not sure how to actually do this
