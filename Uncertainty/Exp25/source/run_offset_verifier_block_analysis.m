...
for fly = 1:length(data_dirs)
    ...
    [x_out,bar_position_to_plot] = removeWrappedLines(data.time,bar_position);
    plot(x_out,bar_position_to_plot,'LineWidth',1.5,'color',[0.2 0.6 0.7])
    hold on
    ...
    plot(x_out,phase_to_plot,'LineWidth',1.5,'color',[0.9 0.3 0.4])
    ...
    plot(x_out,offset_to_plot,'k','LineWidth',1.5)
    xlim([0 data.time(end)]);
    ylim([-180 180]);
    xlabel('Time (sec)');
    ylabel('Deg');
    ...
    mean_verifying_offset = rad2deg(circ_mean(deg2rad(offset)));
