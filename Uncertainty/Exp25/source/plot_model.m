...
subplot(1,2,1)
for fly = 1:length(modelData)
    errorbar([1:2],[beta_visual(fly),beta_visual(fly+length(modelData))],[beta_visual_error(fly),beta_visual_error(fly+length(modelData))],'-o','lineWidth',2)
    hold on
end
...
subplot(1,2,2)
for fly = 1:length(modelData)
    errorbar([1:2],[beta_internal(fly),beta_internal(fly+length(modelData))],[beta_internal_error(fly),beta_internal_error(fly+length(modelData))],'-o','lineWidth',2)
    hold on
end
...
saveas(gcf,[path,'\globalPlots\bump_vel_model_with_SE_OL.png']);
...
for fly = 1:length(modelData)
   ...
   contrast = repelem(1,size(modelData{1,fly}.bump_vel_data_LC,1),1);
   LC_data = addvars(LC_data,contrast);
   ...
   contrast = repelem(2,size(modelData{1,fly}.bump_vel_data_HC,1),1);
   HC_data = addvars(HC_data,contrast);
...
%fit model
bump_vel_model = fitlme(all_data,'BumpAngVel ~ -1 + VisualCueDrive*contrast + FlyAngVel + (1|fly_ID)')
%I'm not sure how to interpret this model