...
%% Take the derivative
    ...
    if (isfield(data,'uncorrectedFicTracAngularPosition') == 1)
        % Repeat the previous processes with the uncorrected heading for the angular velocity
        dataRad_uncorrectedAngularPosition = data.uncorrectedFicTracAngularPosition .* 2 .* pi ./ 10;
        unwrapped_uncorrectedAngularPosition = unwrap(dataRad_uncorrectedAngularPosition);
        downsampled_uncorrectedAngularPosition = resample(unwrapped_uncorrectedAngularPosition,25,sampleRate);
        smoothed.uncorrectedAngularPosition = smoothdata(downsampled_uncorrectedAngularPosition,'rlowess',25);
        uncorrectedAngularPosition_deg = rad2deg(smoothed.uncorrectedAngularPosition); % we transform the angular position to degrees
        uncorrected_shifted_data = uncorrectedAngularPosition_deg - initialAngle;
        %wrap
        uncorrectedAngularPosition_deg = wrapTo360(uncorrected_shifted_data);
        diff_angularPosition = gradient(uncorrectedAngularPosition_deg).* 25;
    else
