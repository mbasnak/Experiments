...
for session = 1:length(open_loop_sessions)
    ...
    %PB heatmap
    subplot(3,5,[1 4])
    dff = data{1,session}.data.dff_matrix;
    imagesc(dff)
    ...
    phase = wrapTo180(rad2deg(data{1,session}.data.phase));
    ...
    stim_offset{session} = rad2deg(circ_dist(data{1,session}.data.phase,deg2rad(data{1,session}.data.panel_angle')));
    stimoffset = rad2deg(circ_dist(data{1,session}.data.phase,deg2rad(data{1,session}.data.panel_angle')));    
    ...
    %Save figures
    saveas(gcf,[path,'\analysis\plots\openLoopHeatmapAndOffset_session',num2str(session),'_oldmethod.png']);
end
...
for session = 1:length(open_loop_sessions)
    ...
    if (data{1,session}.data.run_obj.function_number == 52 | data{1,session}.data.run_obj.function_number == 53)
        summarydata{session,'stim_vel'} = 30; 
    elseif (data{1,session}.data.run_obj.function_number == 54 | data{1,session}.data.run_obj.function_number == 55)
        summarydata{session,'stim_vel'} = 60; 
    elseif (data{1,session}.data.run_obj.function_number == 206 | data{1,session}.data.run_obj.function_number == 207)
        summarydata{session,'stim_vel'} = 90; 
    elseif (data{1,session}.data.run_obj.function_number == 208 | data{1,session}.data.run_obj.function_number == 209)
        summarydata{session,'stim_vel'} = 120; 
    elseif (data{1,session}.data.run_obj.function_number == 210 | data{1,session}.data.run_obj.function_number == 211)
        summarydata{session,'stim_vel'} = 150; 
    elseif (data{1,session}.data.run_obj.function_number == 212 | data{1,session}.data.run_obj.function_number == 213)
...
%save
saveas(gcf,[path,'\analysis\plots\open_loop_offset_var_old_method.png']);
...
%save
saveas(gcf,[path,'\analysis\plots\mean_bump_mag_old_method.png']);
...
%save
saveas(gcf,[path,'\analysis\plots\mean_bump_hw_old_method.png']);
...
for session = 1:length(open_loop_sessions)
   ...
   if (data{1,session}.data.run_obj.function_number == 52 | data{1,session}.data.run_obj.function_number == 53)
       stimVel = [stimVel,repelem(30,length(data{1,session}.data.time))];
   elseif (data{1,session}.data.run_obj.function_number == 54 | data{1,session}.data.run_obj.function_number == 55)
       stimVel = [stimVel,repelem(60,length(data{1,session}.data.time))];
   elseif (data{1,session}.data.run_obj.function_number == 206 | data{1,session}.data.run_obj.function_number == 207)
       stimVel = [stimVel,repelem(90,length(data{1,session}.data.time))];
   elseif (data{1,session}.data.run_obj.function_number == 208 | data{1,session}.data.run_obj.function_number == 209)
       stimVel = [stimVel,repelem(120,length(data{1,session}.data.time))];
   elseif (data{1,session}.data.run_obj.function_number == 210 | data{1,session}.data.run_obj.function_number == 211)
       stimVel = [stimVel,repelem(150,length(data{1,session}.data.time))];
   elseif (data{1,session}.data.run_obj.function_number == 212 | data{1,session}.data.run_obj.function_number == 213)
       stimVel = [stimVel,repelem(180,length(data{1,session}.data.time))];
   elseif (data{1,session}.data.run_obj.function_number == 214 | data{1,session}.data.run_obj.function_number == 215)
       stimVel = [stimVel,repelem(210,length(data{1,session}.data.time))];
   end
end
...
save([path,'\analysis\open_loop_data_old_method.mat'],'bump_mag_data','bump_width_data','summarydata','mean_data');
