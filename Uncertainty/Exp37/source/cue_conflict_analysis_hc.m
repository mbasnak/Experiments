...
load([path,'\analysis\continuous_analysis_sid_',num2str(sessions.cue_conflict_high_contrast),'_tid_0.mat'])
...
%save plot
saveas(gcf,[path,'\analysis\plots\summary_results_high_contrast.png']);
...
%save plot
saveas(gcf,[path,'\analysis\plots\offsets_high_contrast.png']);

%% Change in offset around the jumps
...
for jump = 1:length(j)
   ...
   subplot(2,2,1)
   %bar offset in time
   plot(time_to_plot,bar_offset(j(jump)-25:j(jump)+26))
   hold on
   ...
   subplot(2,2,2)
   %wind offset in time
   plot(time_to_plot,wind_offset(j(jump)-25:j(jump)+26))
   hold on
   xline(time(j(jump)),'k','linestyle','--','linewidth',2)
   title('Wind offset');
   ylim([-180 180]);
   ...
   subplot(2,2,3)
   %change in bar offset
   bar_offset_change = [nanmean(bar_offset(j(jump)-25:j(jump))),nanmean(bar_offset(j(jump)+1:j(jump)+26))];
   plot(bar_offset_change,'-ko')
   ylim([-180 180]);
   xlim([0 3]);
   
   subplot(2,2,4)
   %change in wind offset
   wind_offset_change = [nanmean(wind_offset(j(jump)-25:j(jump))),nanmean(wind_offset(j(jump)+1:j(jump)+26))];
   plot(wind_offset_change,'-ko')
   ylim([-180 180]);
   xlim([0 3]);
...
for window = 1:length(window_sizes)
    ...
    wind_offset_variability(:,window) = matlab.tall.movingWindow(fcn,window_sizes(window),deg2rad(wind_offset));    
end
...
for window = 1:length(window_sizes)
    ...
    plot(continuous_data.time,bar_offset_variability(:,window),'k')
    hold on
    %add bar jumps
    for jump = 1:length(j)
        xline(continuous_data.time(j(jump)),'color',[0.4660, 0.6740, 0.1880],'linewidth',3)
    ...
    if window == 1
        title('window size = 1 s');
        set(gca,'xticklabel',{[]});
    elseif window == 2
        title('window size = 3 s');
    elseif window == 3
        title('window size = 5 s');
    elseif window == 4
        title('window size = 10 s');
    elseif window == 5
        title('window size = 20 s');        
        xlabel('Time (sec)');
    else
        set(gca,'xticklabel',{[]});
    end
...
%% Plot wind offset variability

figure,
for window = 1:length(window_sizes)
    subplot(length(window_sizes),1,window)
    plot(continuous_data.time,wind_offset_variability(:,window),'k')
    hold on
    %add bar jumps
    for jump = 1:length(j)
        xline(continuous_data.time(j(jump)),'color',[0.4660, 0.6740, 0.1880],'linewidth',3)
...
%% Relationship between offsets and offset variabilities

figure,
subplot(121)
plot(bar_offset,wind_offset,'o')
xlabel('Bar offset');
ylabel('Wind offet');

subplot(122)
plot(bar_offset_variability(:,1),wind_offset_variability(:,1),'o')
xlabel('Bar offset variability');
ylabel('Wind offet variability');
...
for jump = 1:length(j)
   ...
   saveas(gcf,[path,'\analysis\plots\AJ_bump_magnitude_hc_',num2str(jump),'.png']);
...
for jump = 1:length(j)
   ...
   saveas(gcf,[path,'\analysis\plots\AJ_bump_width_hc_',num2str(jump),'.png']);
...
saveas(gcf,[path,'\analysis\plots\AJ_bump_parameters_high_contrast.png']);
...
saveas(gcf,[path,'\analysis\plots\AJ_mean_yaw_vel_hc.png']);

%% Forward velocity around the jumps

for jump = 1:length(j)   
   vel_for{jump} = continuous_data.vel_for_ds(j(jump)-250:j(jump)+250);  
       if jumps(jump) == 120
        vel_for_120{jump} = vel_for{jump};
    else
        vel_for_neg120{jump} = vel_for{jump};
    end
end

vel_for_120 =  vel_for_120(~cellfun('isempty',vel_for_120));
vel_for120 = cell2mat(vel_for_120);
vel_for120 = reshape(vel_for120,length(vel_for_120{1}),length(vel_for_120));
vel_for_neg120 =  vel_for_neg120(~cellfun('isempty',vel_for_neg120));
vel_forneg120 = cell2mat(vel_for_neg120);
vel_forneg120 = reshape(vel_forneg120,length(vel_for_neg120{1}),length(vel_for_neg120));

figure,
plot(mean(vel_for120,2),'r')
hold on
plot(mean(vel_forneg120,2),'b')
xline(26,'linewidth',2,'color','k','HandleVisibility','off')
yline(0,'linewidth',2,'color','k','linestyle','--','HandleVisibility','off')
xlim([1 51]);
%ylim([-100 100]);
