...
for folder = 1:length(folderNames)
    if contains(folderNames(folder).name,'60D05')
        data{folder} = load(fullfile(folderNames(folder).folder,folderNames(folder).name,'analysis\all_summary_data_old_method.mat'));
    end
end
...
for folder = 1:length(folderNames)
    if contains(folderNames(folder).name,'60D05')
        load(fullfile(folderNames(folder).folder,folderNames(folder).name,'analysis\all_summary_data_old_method.mat'));
...
save('Z:\Wilson Lab\Mel\Experiments\Uncertainty\Exp33\data\groupPlots\across_flies_analysis_old_method.png');
