...
elseif (optoStatus == 0 & side == 1)
    [opT] = boundedline(distance,nanmean(data(trials,:)),std(data(trials,:)),'-ko','alpha');
elseif (optoStatus == 1 & side == 0)
    [opT] = boundedline(distance,flip(nanmean(data(trials,:))),flip(std(data(trials,:))),'-ro','alpha');
else
    [opT] = boundedline(distance,flip(nanmean(data(trials,:))),flip(std(data(trials,:))),'-ko','alpha');
end
hold on
