...
cd 'Z:\Wilson Lab\Mel\Experiments\PathIntegration\Exp18\data\experimental flies'
dirName = uigetdir();
cd(dirName)
folderNames = dir;
...
for i = 1:size(velocities,1)
    ...
    line([7 7],[0  max(max(probeMeans(i,:)))+1],'Color' ,'r','lineWidth',2);
...
for i = 1:size(velocities,1)
    ...
    line([7 7],[0  max(max(probeAngMeans(i,:)))+10],'Color' ,'r','lineWidth',2);
...
for i = 1:size(optoMeans,1)
    ...
    line([7 7],[min(min([probeMeans;optoMeans]))-0.5 max(max([probeMeans;optoMeans]))+0.5],'Color' ,'r','lineWidth',2);
...
for i = 1:size(probeMeans,1)
    ...
    line([7 7],[min(min([probeMeans;optoMeans]))-0.5 max(max([probeMeans;optoMeans]))+0.5],'Color' ,'r','LineStyle', ':','lineWidth',2);
end
title('Probe trials');
xlabel('y dimension');
ylim([min(min([probeMeans;optoMeans]))-0.5 max(max([probeMeans;optoMeans]))+0.5]);
...
for i = 1:size(optoMeans,1)
    ...
    hold on
    set(h1, 'markerfacecolor', get(h1, 'color')); 
    line([7 7],[min(min([probeAngMeans;optoAngMeans]))-0.5 max(max([probeAngMeans;optoAngMeans]))+0.5],'Color' ,'r','lineWidth',2);
...
for i = 1:size(probeAngMeans,1)
    ...
    hold on
    set(h2, 'markerfacecolor', get(h2, 'color'));
    line([7 7],[min(min([probeAngMeans;optoAngMeans]))-0.5 max(max([probeAngMeans;optoAngMeans]))+0.5],'Color' ,'r','LineStyle', ':','lineWidth',2);
end
title('Probe trials');
xlabel('y dimension');
ylim([min(min([probeAngMeans;optoAngMeans]))-0.5 max(max([probeAngMeans;optoAngMeans]))+0.5]);
...
for i = 1:size(optoMeans,1)
    ...
    hold on
    set(h1, 'markerfacecolor', get(h1, 'color'));
    line([7 7],[0 1],'Color' ,'r','lineWidth',2);
...
for i = 1:size(probeMeans,1)
    ...
    hold on
    set(h2, 'markerfacecolor', get(h2, 'color'));
    line([7 7],[0 1],'Color' ,'r','lineWidth',2,'lineStyle',':');
end
title('Probe trials');
xlabel('y dimension');
ylim([0 1]);
