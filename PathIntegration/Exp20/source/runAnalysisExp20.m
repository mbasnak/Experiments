% Run the analysis of experiment 20 for every trial for every fly and save the plots

clear all; close all;

% Navigate to the experiment data folder
cd 'Z:\Wilson Lab\Mel\Experiments\PathIntegration\Exp20\data\experimental flies'
path = 'Z:\Wilson Lab\Mel\Experiments\PathIntegration\Exp20\data\experimental flies';
            ...
            if strfind(flies(j).name,'20') ~=0
                ...
                for j = 1:length(files)
                    if strfind(files(j).name,'bdata') ~=0
                       ...
                       % Run the analysis
                       singleTrialAnalysisExp20(NewPath,usefulfiles{j})
                    end
                end
