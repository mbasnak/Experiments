...
files = dir('Z:\Wilson Lab\Mel\Experiments\Exp4\data\*\experimental flies\*\dataFromAnalysis\perTrialData.mat');
...
for i = 1:length(files)
    ...
    %my coordiate system was inverted in the x axis in fictrac for this
    %experiment, so I need to negate the forward velocity data to obtain
    %the actual value
...
saveas(gcf,strcat('Z:\Wilson Lab\Mel\Experiments\Exp4\globalPlots\AverageVelocityChange.png'))
...
plot([1,2],[median(around90(1,:)) median(around90(2,:))],'k','lineWidth',2)
...
plot([1,2],[median(aroundNeg90(1,:)) median(aroundNeg90(2,:))],'k','lineWidth',2)
...
plot([1,2],[median(around90Ang(1,:)) median(around90Ang(2,:))],'k','lineWidth',2)
...
plot([1,2],[median(aroundNeg90Ang(1,:)) median(aroundNeg90Ang(2,:))],'k','lineWidth',2)
text(1,max(max(aroundNeg90Ang)),strcat('p = ',num2str(p8)))
set(gca,'xticklabel',{[]})

saveas(gcf,strcat('Z:\Wilson Lab\Mel\Experiments\Exp4\globalPlots\QuantVelChange.png'))
...
Files = dir('Z:\Wilson Lab\Mel\Experiments\Exp4\data\*\experimental flies\*\dataFromAnalysis\goals.mat');
...
saveas(gcf,strcat('Z:\Wilson Lab\Mel\Experiments\Exp4\globalPlots\GoalDistribution.png'))
...
set(h2, 'linewidth', 3);
ylim([0 0.27]);
saveas(gcf,strcat('Z:\Wilson Lab\Mel\Experiments\Exp4\globalPlots\Dist2goal.png'))


%% Regress distance to goal using several variables

% I need to get the FlyData from each folder
Files = dir('Z:\Wilson Lab\Mel\Experiments\Exp4\data\*\experimental flies\*\flyData.mat');
...
Hours(i) = hour(datetime(TimeStamp{i}));
Day(i) = day(datetime(TimeStamp{i}));
...
saveas(gcf,strcat('Z:\Wilson Lab\Mel\Experiments\Exp4\globalPlots\FactorEffects.png'))


%% Distance to the goal in the 100 sec around the jumps

Files = dir('Z:\Wilson Lab\Mel\Experiments\Exp4\data\*\experimental flies\*\dataFromAnalysis\shortData.mat');
...
saveas(gcf,strcat('Z:\Wilson Lab\Mel\Experiments\Exp4\globalPlots\Dist2Goal100sec.png'))

%% Distance to the goal distribution for empty trials


Files = dir('Z:\Wilson Lab\Mel\Experiments\Exp4\data\*\experimental flies\*\dataFromAnalysis\EmptyTrialGoals.mat');
...
saveas(gcf,strcat('Z:\Wilson Lab\Mel\Experiments\Exp4\globalPlots\GoalDistributionEmptyTrial.png'))
...
saveas(gcf,strcat('Z:\Wilson Lab\Mel\Experiments\Exp4\globalPlots\Dist2goalEmmptyTrial.png'))
